...
func (m *Manager) Edit(_ context.Context, f extract.File, up model.Update) (model.Edit, error) {
	...
	verify := func(start, end int, want string) error {
		if start < 0 || end < 0 || start > end || end > len(src) {
			return fmt.Errorf("dockerfile: %s: locus [%d:%d] is out of range (file has %d bytes)", f.Path, start, end, len(src))
		}
		if got := string(src[start:end]); got != want {
			return fmt.Errorf("dockerfile: %s changed since extraction: expected %q at [%d:%d], found %q", f.Path, want, start, end, got)
		}
		return nil
	}

	hasDigest := dep.Locus.DigestStart != model.NoDigest && dep.Locus.DigestEnd != model.NoDigest
	valueChanges := up.NewValue != dep.CurrentValue
	digestChanges := up.NewDigest != "" && up.NewDigest != dep.CurrentDigest

	switch {
	case hasDigest && valueChanges && digestChanges:
		// A combined tag+digest bump. The two loci must be contiguous -
		// "tag@digest" - for one byte range to cover both.
		if err := verify(dep.Locus.ValueStart, dep.Locus.ValueEnd, dep.CurrentValue); err != nil {
			return model.Edit{}, err
		}
		if err := verify(dep.Locus.DigestStart, dep.Locus.DigestEnd, dep.CurrentDigest); err != nil {
			return model.Edit{}, err
		}
		if dep.Locus.DigestStart != dep.Locus.ValueEnd+1 || src[dep.Locus.ValueEnd] != '@' {
			return model.Edit{}, fmt.Errorf("dockerfile: %s: value and digest loci are not contiguous; refusing a combined edit", f.Path)
		}
		return model.Edit{
			File:    f.Path,
			Start:   dep.Locus.ValueStart,
			End:     dep.Locus.DigestEnd,
			Old:     dep.CurrentValue + "@" + dep.CurrentDigest,
			New:     up.NewValue + "@" + up.NewDigest,
			Manager: name,
		}, nil
	...
	case hasDigest && digestChanges:
		...
		return model.Edit{
			File:    f.Path,
			Start:   dep.Locus.DigestStart,
			End:     dep.Locus.DigestEnd,
			Old:     dep.CurrentDigest,
			New:     up.NewDigest,
			Manager: name,
		}, nil

	case !hasDigest && up.NewDigest != "":
		// Pinning a digest onto a reference that previously had none: the
		// value locus is replaced by "value@newDigest" in one edit, since
		// there is no separate digest locus yet to target.
		if err := verify(dep.Locus.ValueStart, dep.Locus.ValueEnd, dep.CurrentValue); err != nil {
			return model.Edit{}, err
		}
		...
		return model.Edit{
			File:    f.Path,
			Start:   dep.Locus.ValueStart,
			End:     dep.Locus.ValueEnd,
			Old:     dep.CurrentValue,
			New:     newValue + "@" + up.NewDigest,
			Manager: name,
		}, nil
	...
	default:
		return model.Edit{}, fmt.Errorf("dockerfile: update for %s names no change to apply", dep.DepName)
	}
}
