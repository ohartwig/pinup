...
// Datasource is the git-tags datasource.
type Datasource struct {
	...
	lsRemote func(ctx context.Context, gitBin string, timeout time.Duration, repoURL string) (string, error)
}

// New returns a git-tags datasource that execs the real git binary.
func New() *Datasource {
	return &Datasource{lsRemote: execLsRemote}
}

func (d *Datasource) Name() string { return "git-tags" }
...
func (d *Datasource) Releases(ctx context.Context, ref lookup.Ref) (*model.ReleaseSet, error) {
	...
	out, err := ls(ctx, d.Git, timeout, repoURL)
	if err != nil {
		return nil, fmt.Errorf("git-tags: ls-remote %s: %w", redactURL(repoURL), err)
	}
...
func execLsRemote(ctx context.Context, gitBin string, timeout time.Duration, repoURL string) (string, error) {
	bin := gitBin
	if bin == "" {
		bin = "git"
	}
	...
	cmd := exec.CommandContext(cctx, bin, "ls-remote", "--tags", "--refs", repoURL)
