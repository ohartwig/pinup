...
var cases = []struct {
	...
	{name: "a repository switched off has nothing to advise on", cfg: `{"enabled": false, "nonesuchKey": 1}`,
		want: "compat/key-unsupported", pointer: "/nonesuchKey", fix: &Fix{Pointer: "/nonesuchKey", Op: OpRemove, Changes: []string{"/nonesuchKey"}},
		absent: []string{"sec/minimum-release-age-unset", "sec/vulnerability-alerts-off", "sec/pin-digests-off"}},
